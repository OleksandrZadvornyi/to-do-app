...
describe('SimplyDone Todo App', () => {
    ...
    it('should add a new todo item', async () => {
        ...
        await expect(TodoPage.listItems[0]).toHaveText(expect.stringContaining('Buy groceries'));
    });
    ...
    it('should filter active and completed tasks', async () => {
        ...
        await expect(TodoPage.listItems).toBeElementsArrayOfSize(1);
        await expect(TodoPage.listItems[0]).toHaveText(expect.stringContaining('Active Task'));

        // 3. Filter by Completed
        await TodoPage.btnFilterCompleted.click();
        await expect(TodoPage.listItems).toBeElementsArrayOfSize(1);
        await expect(TodoPage.listItems[0]).toHaveText(expect.stringContaining('Completed Task'));
    });
});